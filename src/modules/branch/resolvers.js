    ...
    Mutation: {
        ...
        changeBranch: async (_, args, global) => {
            try {
                ...
                if (!result.toall && !result.change_branch.split('-').includes(args.branchId.toString())) {throw new NotFoundError('You can\'t change this branch')}
        ...
        deleteBranch: async (_, args, global) => {
            try {
                ...
                if (!result.toall && !result.delete_branch.split('-').includes(args.branchId.toString())) {throw new NotFoundError('You can\'t delete branch')}
